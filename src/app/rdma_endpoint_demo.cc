...
void ServerThreadMain(volatile char *buffer, RdmaEndpoint &ep, int client_id) {
    ...
    uint64_t recv_wr_id = ep.Recv(client_id, base_offset + kRecvOffset, 3);

    LOG(INFO) << "Wait 3 seconds for client " << client_id << " to post RECV";
    std::this_thread::sleep_for(3s);

    ep.Write(false, client_id, base_offset + 5, 5, 5, 0 /* unsignaled */);
    uint64_t send_wr_id = ep.Send(client_id, base_offset + 10, 4,
                 IBV_SEND_SIGNALED | IBV_SEND_FENCE /* wait for previous WRITE to complete */);
    ep.WaitForCompletion(client_id, true, send_wr_id);

    // Wait for client response
    ep.WaitForCompletion(client_id, true, recv_wr_id);
    char expected[4];
...
void ServerMain() {
    ...
    RdmaEndpoint ep(nullptr, 0, reinterpret_cast<volatile unsigned char *>(buffer), kBufferSize, 128, 128, IBV_QPT_RC);
    ...
    for (int cid = 0; cid < FLAGS_clients; cid++) {
        threads.push_back(std::thread(ServerThreadMain, buffer, std::ref(ep), cid));
    }
...
void ClientMain() {
    volatile char *buffer = new volatile char[kBufferSize]();
    RdmaEndpoint ep(nullptr, 0, reinterpret_cast<volatile unsigned char *>(buffer), kBufferSize,
                       128, 128, IBV_QPT_RC);
                    ep.Connect(FLAGS_endpoint.c_str());
    LOG(INFO) << "Connected to peer";

    // Receive "sXXX" at offset 10, where XXX is client id (padded with 0 at front)
    uint64_t recv_wr_id = ep.Recv(0, 10, 4);
    ep.WaitForCompletion(0, true, recv_wr_id);
    // Read "rXXX " at offset 0
    uint64_t read_wr_id = ep.Read(0, 0, FLAGS_client_id * kClientMemoryStride, 5);
    ep.WaitForCompletion(0, true, read_wr_id);

    uint64_t send_wr_id = ep.Send(0, 1, 3);
    ep.WaitForCompletion(0, true, send_wr_id);
